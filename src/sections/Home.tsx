...
export default function Home({ data }: { data: HomeApiResponse }) {
	...
	return (
		<div
			className={`animate-in fade-in slide-in-from-bottom-4 duration-700 ${focusMode ? "text-center min-h-[calc(50vh-8rem)] max-h-[70vh] overflow-y-hidden flex flex-col justify-center" : ""}`}
		>
			<section className={`${focusMode ? "mb-0" : "mb-12 md:mb-32"}`}>
				...
				<h1
					className={`text-5xl md:text-8xl font-bold tracking-tight mb-8 leading-[0.85] transition-colors ${focusMode ? "text-white" : "text-[#1A1A1A]"}`}
				>
					{data.name.first} <br />
					<span
						className={`italic transition-colors ${focusMode ? "text-white/60" : "text-[#0E4D47]"}`}
					>
						{data.name.last}.
					</span>
				</h1>
				...
				<div
					className={`text-xl font-mono leading-relaxed ${focusMode ? "mx-auto" : "max-w-2xl"}`}
				>
				...
				<section className="grid md:grid-cols-2 gap-8 md:gap-12 border-t pt-8 md:pt-12 transition-colors border-black/[0.03]">
					<div>
						...
						<ul className="space-y-3 font-mono text-sm">
							{technicalCategories.map((cat) => (
								<li
									key={cat.label}
									className="flex justify-between border-b pb-1 border-black/[0.02]"
								>
									<span>{cat.label}:</span>
					...
					<div className="flex-col mt-4 md:mt-0 hidden md:flex">
						<p className="text-gray-500 text-sm mb-6 leading-relaxed">
							{data.phrase ? (
								<>
									<span className="text-gray-400 italic">Current phrase:</span>{" "}
									“{data.phrase}”
								</>
							) : (
